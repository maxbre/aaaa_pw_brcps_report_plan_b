...
*Valutazione di impatto sanitario dell’inquinamento atmosferico da $NO_2$: scenario controfattuale e propagazione dell’incertezza. Il caso della Regione Veneto.*
...
*Health impact assessment of ambient $NO_2$ pollution: counterfactual scenario and uncertainty propagation. A case study of the Veneto Region.*
...
$$
La derivata prima positiva indica che la frazione attribuibile aumenta
all'aumentare del differenziale di esposizione; a parità di decessi
attesi, aumentano di conseguenza anche i casi attribuibili. La derivata
seconda negativa indica che la funzione è strettamente concava e presenta
incrementi marginali decrescenti.
...
La *disuguaglianza di Jensen* [@casella2002statistical] mette in
relazione il valore atteso di una funzione con la funzione del valore
atteso. Se indichiamo con $\varphi$ una funzione concava e con $X$ una
variabile casuale con valore atteso finito, vale:
...
Poiché $f(\Delta C)$ è strettamente concava sul dominio
$\Delta C\geq0$, la disuguaglianza può essere applicata ai differenziali
non negativi tra le unità territoriali:
...
Per applicare questa relazione alla frazione attribuibile regionale è
necessario specificare rispetto a quali pesi siano calcolati i valori
attesi. Una media aritmetica attribuirebbe la stessa importanza a ogni
sezione censuaria, indipendentemente dalla popolazione e dalla mortalità
basale, e non corrisponderebbe pertanto all'indicatore regionale
utilizzato nello studio.

Infatti, poiché per ciascuna sezione:

$$
AC_i=E_i f(\Delta C_i),
$$

la frazione attribuibile regionale è:

$$
AF_{\mathrm{Reg}}
=
\frac{\sum_i AC_i}{\sum_i E_i}
=
\frac{\sum_i E_i f(\Delta C_i)}{\sum_i E_i}.
$$
...
AF_{\mathrm{Reg}}
=
\sum_i w_i f(\Delta C_i).
$$

I pesi non sono dunque introdotti arbitrariamente per applicare la
disuguaglianza di Jensen, ma derivano direttamente dalla definizione
della frazione attribuibile regionale. Essi esprimono il contributo di
ciascuna sezione al totale regionale dei decessi attesi.

Considerando la distribuzione discreta che assegna alla sezione $i$ il
peso $w_i$, i due valori attesi ponderati sono:

$$
\mathbb{E}_w[f(\Delta C)]=\sum_i w_i f(\Delta C_i),
\qquad \text{e} \qquad
\mathbb{E}_w[\Delta C]=\sum_i w_i\Delta C_i.
$$
La forma ponderata della disuguaglianza di Jensen diventa pertanto:

$$
\sum_i w_i f(\Delta C_i)
\leq
f\left(\sum_i w_i\Delta C_i\right).
$$

La relazione mette a confronto due strategie:

- **$\sum_i w_i f(\Delta C_i)$**: la frazione attribuibile calcolata
  separatamente per ciascuna sezione e successivamente aggregata
  utilizzando come pesi i decessi attesi;

- **$f\left(\sum_i w_i\Delta C_i\right)$**: la frazione calcolata una
  sola volta sul differenziale medio ponderato, trattando l'area come
  omogenea.
...
con media aritmetica $\mathbb{E}[f(\Delta C)] \approx 0{.}0923$.

Invece: 
$f(\overline{\Delta C}) = f(10) = 1 - e^{-0{.}1} \approx 0{.}0952$
cioè, il valore ottenuto applicando la funzione al differenziale medio risulta, seppure di poco, superiore alla media dei valori calcolati separatamente.
