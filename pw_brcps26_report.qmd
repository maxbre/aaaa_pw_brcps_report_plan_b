...
Nell'assetto informativo attualmente disponibile per lo studio non è, tuttavia, presente una serie completa di tassi provinciali di mortalità respiratoria età-specifici, coerente per definizione dell'esito, classi d'età, periodo e denominatori demografici. Non è stato quindi possibile applicare una standardizzazione indiretta correttamente allineata ai dati di popolazione sezionale. La scelta non dipende dalla scarsa numerosità dei decessi osservati nelle singole sezioni: una standardizzazione indiretta impiegherebbe infatti tassi esterni e non richiederebbe di stimare tassi sezionali instabili.
...
Nella prima, riferita specificamente al 2025, le concentrazioni modellate, la popolazione e i tassi di mortalità sono stati mantenuti fissi. A ogni iterazione è stato estratto un unico valore di $RR_{10}$, comune a tutte le sezioni, assumendo:

$$
\log(RR_{10})\sim\mathcal{N}\!\left[\log(1{.}05),\,\sigma_{\log RR}^{2}\right],
\qquad
\sigma_{\log RR}=\frac{\log(1{.}07)-\log(1{.}03)}{2\times1{.}96}.
$$

La distribuzione risultante propaga esclusivamente l'incertezza epidemiologica associata al $RR$. I percentili 2,5 e 97,5 possono quindi essere interpretati come limiti di un IC 95% dell'impatto propagato dal $RR$, condizionato agli altri input trattati come noti.

Nella seconda analisi, finalizzata a descrivere la variabilità storica del carico sanitario, a ogni iterazione è stato selezionato con reinserimento e con uguale probabilità uno degli scenari annuali di esposizione 2019–2025. Lo stesso anno è stato applicato simultaneamente a tutte le sezioni, preservandone la configurazione spaziale modellata, ed è stato estratto un valore del $RR$ dalla medesima distribuzione lognormale. Popolazione e tassi basali sono rimasti fissi. I percentili ottenuti costituiscono un *intervallo di simulazione* riferito alla combinazione tra variabilità storica dell'esposizione e incertezza epidemiologica; non sono un IC della stima 2025 né una previsione.
...
La linea tratteggiata rossa identifica la mediana simulata, pari a circa 109 casi/anno. Le linee verdi delimitano l'IC 95% propagato dal $RR$: circa 68 casi/anno al 2,5° percentile e 150 casi/anno al 97,5° percentile. L'area ombreggiata racchiude la massa di probabilità centrale corrispondente.
...
Oltre al $RR$, potranno essere incluse l'incertezza delle concentrazioni modellate, dei tassi epidemiologici di base e delle stime demografiche sezionali. L'errore delle concentrazioni potrebbe essere caratterizzato confrontando le stime modellistiche con una granularità più fine, da realizzare attraverso tecniche di data fusion con le misure delle stazioni di monitoraggio di qualità dell'aria e propagando i residui dotati di struttura spaziale. 
Qualora fossero disponibili conteggi e denominatori sufficientemente dettagliati, i tassi basali potrebbero essere stabilizzati mediante modelli gerarchici o tecniche di *Empirical Bayes smoothing* [@blangiardo_cameletti:2015]. Queste tecniche sarebbero utili per stime locali derivate da dati sparsi, ma non sostituiscono l'indisponibilità delle necessarie informazioni età-specifiche.
...
La derivata prima positiva indica che la funzione è monotonicamente crescente: all'aumentare del differenziale di esposizione aumentano i casi attribuibili. La derivata seconda negativa indica che la funzione è strettamente concava e presenta incrementi marginali decrescenti.

In altri termini, a parità delle altre condizioni, incrementi successivi di uguale ampiezza in $\Delta C$ producono aumenti progressivamente più contenuti della frazione attribuibile.
...
Poiché $f(\Delta C)$ è strettamente concava sul dominio $\Delta C\geq0$, la disuguaglianza può essere applicata ai differenziali non negativi tra le unità territoriali:

$$E[f(\Delta C)] \le f(E[\Delta C])$$
Con pesi $w_i$ proporzionali ai decessi attesi ($w_i=E_i/\sum_iE_i$), il confronto pertinente è:

$$
\sum_iw_i f(\Delta C_i)\leq f\!\left(\sum_iw_i\Delta C_i\right).
$$

La relazione mette a confronto due strategie:

- **$\sum_iw_i f(\Delta C_i)$**: la frazione attribuibile calcolata separatamente per ciascuna sezione e successivamente aggregata con i pesi dei decessi attesi;

- **$f(\sum_iw_i\Delta C_i)$**: la frazione calcolata una sola volta sul differenziale medio ponderato, trattando l'area come omogenea.

Con gli stessi pesi e con $\Delta C_i$ già definito mediante il troncamento a zero, la prima quantità è minore o uguale alla seconda.

L'uguaglianza vale quando il differenziale è costante nelle unità con peso positivo. La conclusione non è automaticamente estendibile a procedure diverse, per esempio alla media delle concentrazioni grezze calcolata prima dell'applicazione di $\max(C-C_{\mathrm{ref}},0)$, oppure a confronti che usano pesi differenti.

L'applicazione sezionale rimane quindi preferibile perché conserva l'eterogeneità disponibile e utilizza correttamente i decessi attesi come pesi; la disuguaglianza di Jensen descrive una specifica conseguenza matematica dell'aggregazione, non una regola universale sulla direzione di ogni possibile errore ecologico.
...
Nell'esempio, a pesi uguali, lo scarto deriva dalla concavità della funzione. Nei dati reali l'ampiezza e la direzione della differenza dipendono anche dalla definizione del differenziale, dai pesi basali e dal livello al quale viene effettuata l'aggregazione.
