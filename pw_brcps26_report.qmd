...
*Valutazione di impatto sanitario da $\text{NO}_2$ atmosferico: analisi controfattuale ed incertezza di stima. Il caso studio della Regione Veneto.*
...
*Health Impact Assessment of Atmospheric $\text{NO}_2$: Counterfactual Analysis and Estimation Uncertainty. The Case Study of the Veneto Region.*
...
- *epidemiologica*, epidemiologica, valutata estraendo, ad ogni iterazione di bootstrap, il Rischio Relativo ($RR$) da una distribuzione log-normale incentrata sul valore centrale OMS ($RR = 1{.}05$);

- *temporale*, valutata tramite l'introduzione della "perturbazione ambientale" conseguente all'estrazione casuale con reinserimento del valore medio annuale di $\text{NO}_2$ per ciascuna sezione censuaria, riferito alla serie storica 2019-2025.
...
Nel ricampionamento con reinserimento delle unità territoriali (sezioni di censimento), la somma delle popolazioni delle unità estratte in ogni iterazione ($P_{\text{boot}}$) varia inevitabilmente attorno alla media. Poiché le sezioni censuarie presentano dimensioni demografiche differenti, l'estrazione casuale di un numero maggiore di sezioni ad alta o bassa densità abitativa altera la dimensione del bacino di popolazione complessivo. 
Se quindi i casi attribuibili ($AC$) venissero calcolati direttamente su $P_{\text{boot}}$, l'ampiezza dell'intervallo di confidenza finale rifletterebbe un artefatto del campionamento demografico, anziché la vera incertezza dell'impatto sanitario.
...
La popolazione 30+ di ogni unità estratta viene riproporzionata ($P_{i, \text{cal}} = P_i \cdot w_b$), garantendo che in ogni replica valga esattamente il vincolo:$\sum_{i} P_{i, \text{cal}} = P_{\text{regionale}}$.
...
E' necessario precisare che gli estremi $\text{AC}_{\text{low}}$ e $\text{AC}_{\text{upp}}$ riportati nella @tbl-ac-prov rappresentano dei valori deterministici, calcolati direttamente a partire dall'intervallo di confidenza del $RR$ da letteratura OMS ($1.03 - 1.07$). 

Per tentare di superare la rigidità di questa stima di tipo deterministico ed estendere l'analisi ad una distribuzione empirica della variabilità di stima, è stata implementata la procedura di *non-parametric bootstrap* descritta nella sezione @sec-methods. 

L'approccio deterministico fornisce una stima puntuale dei casi attribuibili ($AC$) mentre l'applicazione del bootstrap non parametrico permette di propagare l'incertezza parametrica del rischio relativo ($RR$) e della variabilità temporale delle esposizioni, consentendo una caratterizzazione probabilistica dell'impatto sanitario e fornendo intervalli di confidenza al $95\%$ indispensabili per una robusta valutazione del rischio in Sanità Pubblica.

La @fig-boot-spattemp illustra la curva di densità di probabilità associata alla distribuzione empirica dei casi attribuibili generata con il processo di bootstrap non parametrico. La linea tratteggiata rossa identifica la stima centrale della distribuzione pari a circa 148 casi/anno. Le due linee verticali tratteggiate in verde delimitano l'intervallo di confidenza empirico al 95%: l'estremo inferiore ($\text{IC } 95\%\text{ inf.}$) si attesta in circa 86 casi/anno ($2{.}5^\circ$ percentile), mentre l'estremo superiore ($\text{IC } 95\%\text{ sup.}$) raggiunge circa 211 casi/anno ($97{.}5^\circ$ percentile). L'area ombreggiata sottesa alla curva racchiude la massa di probabilità centrale corrispondente all'intervallo di confidenza del 95%. Infine, la linea punteggiata (in grigio) corrispondente a circa 109 casi/anno rappresenta l'entità della stima deterministica puntuale discussa in precedenza per l'anno 2025.


```{r}
#| label: fig-boot-spattemp
#| fig-cap: "Stima bootstrap (AC) casi attribuibili/anno in Regione Veneto."
#| fig-align: "center"
#| out-height: "0.4\\textheight"
#| out-width: "!\\textheight"
#| echo: false
...
L'incremento del valore centrale della distribuzione bootstrap (circa $148$ casi/anno) rispetto alla stima deterministica riferita all'anno 2025 (circa $109$ casi/anno) è riconducibile all'effetto della variabilità introdotta con il processo di bootstrapping e determinata dall'inclusione delle concentrazioni di $\text{NO}_2$ degli anni precedenti (2019-2024), storicamente più elevate rispetto al 2025, integrata con la propagazione *log-normale* del rischio epidemiologico $RR$ che ha l'effetto complessivo di traslare la distribuzione verso valori di impatto più elevati.

I percentili $2{.}5^\circ$ e $97{.}5^\circ$ della distribuzione empirica generata con il processo di bootstrap definiscono l'intervallo di confidenza al 95% (IC 95%) e forniscono una quantificazione dell'incertezza più robusta rispetto ai calcoli deterministici fissi riferiti al solo anno 2025.
...
Dal punto di vista epidemiologico, la stima ottenuta con la valutazione di impatto sanitario rappresenta il numero di *casi/anno teorici*, cioè i decessi evitabili nell'eventualità in cui tutte le sezioni censuarie della Regione Veneto siano esposte ad un livello di concentrazione ambientale di $NO_2 \leq 10\ \mu g~m^{-3}$, che rappresenta la soglia OMS stabilita dalle attuali evidenze scientifiche per escludere significativi (misurabili) effetti sanitari (malattie per cause respiratorie codici ICD-10: J00-J99).

Il confronto tra la stima puntuale riferita all'anno 2025 (circa 109 casi/anno) e la stima centrale ottenuta dalla procedura di bootstrap non parametrico (circa 148 casi/anno, IC 95% 86–211) mette in luce un aspetto metodologico significativo: affidarsi al solo dato deterministico dell'ultimo anno disponibile rischia di sottostimare il carico sanitario atteso, poiché non tiene conto né della variabilità storica dell'esposizione (concentrazioni di NO2 nel periodo 2019-2025, sistematicamente più elevate nei primi anni della serie), né dell'incertezza epidemiologica associata al Rischio Relativo ($RR$).

Da un punto di vista di supporto alle decisioni di Sanità Pubblica, la stima più prudente e metodologicamente robusta da comunicare ai decisori è quella ottenuta dal bootstrap che integra le due fonti di incertezza (epidemiologica, temporale).
