...
Lo studio si propone una valutazione controfattuale di impatto sanitario (VIS) del numero di eventi (casi attribuibili) conseguenti all'esposizione a lungo termine (media annuale) ai livelli ambientali di $NO_2$, attraverso l'impiego dell'evidenza causale di letteratura (*Risk Ratio*) [@hrapie-2:2025], applicata alla popolazione residente nelle sezioni censuarie ISTAT in Regione Veneto.

In assenza di dati individuali georeferenziati - di fatto inaccessibili per le stringenti normative di tutela della *privacy*  -, la stima si avvale delle *sezioni censuarie ISTAT* come unità fondamentale di analisi perchè rappresentano il migliore livello di granularità spaziale disponibile (accessibile) ed offrono un buon compromesso computazionale ed epidemiologico (*meso*) per approssimare l'esposizione all'$\text{NO}_2$ e cercare limitare i fattori di confondimento locali. L'interpretazione dei risultati deve comunque tenere conto della natura *aggregata* dei dati, riferendo sempre le valutazioni di impatto sanitario a livello *aggregato* di popolazione e territorio (*ecological effect*), evitando sempre l'erronea inferenza diretta a livello individuale (*ecological fallacy*).

L'obiettivo secondario è delineare un possibile percorso di sviluppo metodologico per stimare in modo robusto il numero di *casi attribuibili* associati all'inquinamento atmosferico da *biossido di azoto* $NO_2$ e riconducibili a specifici scenari controfattuali in Regione Veneto. 
In prospettiva futura lo studio rappresenta la base per definire uno strumento operativo di analisi quantitativa e di supporto decisionale alla programmazione ed alla valutazione delle Politiche Regionali in materia di Ambiente e Sanità Pubblica.
...
In Regione Veneto sono presenti 43447 sezioni censuarie abitate secondo il censimento ISTAT 2021 (@fig-map-sez21). Le sezioni censuarie abitate rappresentano un totale di 4847745 residenti, di cui 3521995 nella classe 30+ (circa il 73% della popolazione) e coprono una superficie complessiva di 14511 $km^2$, con una densità media di 334 $ab~km^2$. La sezione censuaria *mediana* in Regione Veneto ha un'estensione pari a circa 2.90 $ha$ [^nota-ha], con una densità abitativa *mediana* pari a 13.5 $ab~ha^{-1}$.

[^nota-ha]: Superficie corrispondente ad un quadrato con lato di circa 170 m. 

In @fig-map-pop30p viene riprodotta la distribuzione spaziale della popolazione in classe d'età 30+ residente nelle sezioni censuarie della Regione Veneto (anno 2021) mentre lLa @fig-map-attesi rappresenta per le sezioni censuarie del Veneto il numero di casi attesi/anno per cause respiratorie (codici ICD-10: J00-J99), cioè il numero di decessi teoricamente osservabili ogni anno calcolati sulla base della popolazione esposta (sezioni censuarie ISTAT anno 2021) ed i tassi grezzi provinciali (ISTAT anno 2024 per la corrispondente causa di mortalità).

E' necessario precisare che le mappe in @fig-map-pop30p e @fig-map-attesi sono delle rappresentazioni geografiche di tipo demografico-attuariale che restituiscono la popolazione *target* (a rischio) rispetto alla quale è stata effettuata la valutazione di impatto sanitario (conteggio dei casi attribuibili).

In @fig-map-no2-2025 viene rappresentata la distribuzione spaziale della concentrazione media annuale di NO2 nella Regione Veneto per l'anno 2025 prodotta su grigliato di regolare e risoluzione orizzontale di 4 km tramite la catena modellistica SPIAIR. Il 2025 è l'anno più recente per cui sono disponibili stime modellistiche aggiornate e rappresenta il determinante ambientale rispetto al quale viene effettuata la valutazione di impatto sanitario sulla popolazione residente (attesi) in classe d'età 30+. 

Per un inquadramento della serie storica delle concentrazioni ambientali di $NO2$ stimate per il periodo dal 2019 al 2025  dalla catena modellistica implementata nella Regione Veneto ed una valutazione del trend annuale si rimanda ai grafici riportati in Appendice (@fig-map-no2-ts e @fig-map-mk-sen).
...
La mappa rappresenta il livello di esposizione alle concentrazioni ambientali di $NO_2$ dell'anno 2025 per la popolazione in classe d'età 30+ residente in ciascuna sezione censuaria.
Per la Regione Veneto la popolazione delle sezioni censuarie risulta esposta ad una concentrazione media pesata (*PWE population weighted exposure*) di $NO_2$ pari a $16\ \mu g~m^{-3}$ mentre il livello medio pesato a livello provinciale varia nell'intervallo compreso tra circa $7\ \mu g~m^{-3}$, per la Provincia di Belluno, e circa $18\ \mu g~m^{-3}$, per le Province di Padova e Venezia.

Per un inquadramento dei valori di concentrazione ambientale di $NO_2$ rispetto alla serie storica registrata dal 2019 al 2025 si rinvia al grafico in @fig-no2-boxplot-prov che riporta anche l'indicazione del valore target OMS $10\ \mu g~m^{-3}$ utilizzato nella presente valutazione di impatto sanitario come soglia controfattuale.

Per contestualizzare in modo più preciso l'esposizione della popolazione 30+ residente nelle sezioni censuarie del Veneto rispetto ai livelli ambientali di $NO2$, in @fig-exp-pop_prov viene riprodotta la curva cumulata della percentuale di popolazione soggetta ai livelli di concentrazione ambientale. Nel grafico sono riprodotte per semplicità solo le curve relative al 2019 e 2025 stratificate per Provincia che evidenziano un significativo miglioramento del livello di esposizione nel corso del tempo e contemporaneamente definiscono il "margine" di miglioramento della qualità dell'aria necessario per raggiungere il controfattuale OMS di $10\ \mu g~m^{-3}$.

```{r}
#| label: fig-map-sez21
#| fig-cap: "Mappa sezioni censuarie ISTAT 2021 Regione Veneto."
#| fig-align: "center"
...
#| fig-cap: "Mappa popolazione classe 30+ sezioni censuarie Regione Veneto."
#| fig-align: "center"
#| out-height: "0.45\\textheight"
#| out-width: "!\\textheight"
#| echo: false
...
#| fig-cap: "Mappa casi attesi sezioni censuarie Regione Veneto."
#| fig-align: "center"
#| out-height: "0.45\\textheight"
#| out-width: "!\\textheight"
#| echo: false
...
#| fig-cap: "Mappa concentrazione media annuale NO2 da modellistica Regione Veneto."
#| fig-align: "center"
#| out-height: "0.45\\textheight"
#| out-width: "!\\textheight"
#| echo: false
...
#| fig-cap: "Mappa concentrazioni media NO2 anno 2025 per sezioni censuarie Regione Veneto."
#| fig-align: "center"
#| out-height: "0.45\\textheight"
#| out-width: "!\\textheight"
#| echo: false
...
#| fig-cap: "Boxplot concentrazioni NO2 anno 2025 per sezioni censuarie  Regione Veneto."
#| fig-align: "center"
#| out-height: "0.45\\textheight"
#| out-width: "!\\textheight"
#| echo: false
...
#| fig-cap: "Esposizione cumulata popolazione ai livelli ambeintali NO2."
#| fig-align: "center"
#| out-height: "0.45\\textheight"
#| out-width: "!\\textheight"
#| echo: false
...
La stima di impatto sanitario [^nota-formula] per l'anno di riferimento 2025 in tutta la Regione Veneto ha restituito una *stima puntuale* pari a *109 casi attribuibili/anno per cause respiratorie (codici ICD-10: J00-J99)* nella classe d'età 30+, in relazione al potenziale raggiungimento della soglia OMS di $10\ \mu g~m^{-3}$ utilizzata come soglia controfattuale. 
...
La stima *centrale* di tipo puntuale $RR = 1.05$ rende conto complessivamente per tutta la Regione Veneto di una *frazione attribuibile (AF) pari al 2.86% dei casi attesi*. Considerando gli estremi superiore ed inferiore del $R$ da letteratura internazionale OMS ($1.03 - 1.07$) il numero di casi attribuibili/anno varia da 66.3 (limite inferiore AF = 1.74%) a 150 (limite superiore AF = 3.95%).

Nella @tbl-province viene riportato per l'anno 2025 il dettaglio per Provincia della Regione Veneto del numero di casi attesi, del valore centrale (AC) dell'estremo inferiore (AC low) e superiore (AC upp) dei casi attribuibili e della frazione attribuibile (AF).

| Provincia | Attesi | AC | AF | AC low | AC uppp |
| :--- | ---: | ---: | ---: | ---: | ---: |
...
: Stime dei casi attribuibili (anno 2025) per Provincia {#tbl-province}

Dal punto di vista dell'intrepretazione epidemiologica la stima puntale ottenuta dalla valutazione di impatto sanitario rappresenta il numero di *casi/anno teorici (decessi evitabili)* nell'eventualità per cui tutte le sezioni censuarie del Veneto siano esposte ad un livello di concentrazione ambientale di $NO_2 < 10\ \mu g~m^{-3}$, che rappresenta la soglia prestabilita dalle attuali evidenze scientifiche per escludere significativi (misurabili) effetti sanitari.



- Bootstrapping intervallo di confidenza stima puntuale VIS
...
2. The assumption of a single, well-defined version for each treatment. under the SUTVA assumption, administering drug A with a lower dosage is considered a different treatment to the administration of the identical drug A but with a higher dosage.
...
In @fig-map-no2-ts sono riprodotte le mappe di distribuzione spaziale delle concentrazioni medie annuali di $NO2$. Il confronto tra gli anni evidenzia un sensibile e stabile decremento nel tempo con i valori medi più bassi registrati nell'anno 2025.
...
#| fig-cap: "Mappa trend annuale NO2 comunale (2019-2025). Per una dettagliata descrizione dei colori e della simbologia utilizzata nella rappresentazione si rimanda al testo."
#| fig-align: "center"
#| out-height: "0.45\\textheight"
#| out-width: "!\\textheight"
#| echo: false
...
È importante sottolineare che l'analisi del trend mediante il test di Mann-Kendall ed il calcolo della pendenza non parametrica della retta di Sen è stata condotta *separatamente per ciascun Comune* della Regione Veneto, cioè trattando ogni serie temporale come indipendente dalle altre. L'applicazione del test ha la finalità esclusiva di individuare la presenza, la direzione e l'entità di un trend temporale all'interno della singola unità territoriale, e *non tiene conto della eventuale correlazione o dipendenza spaziale tra Comuni limitrofi*.

Un'eventuale struttura di autocorrelazione spaziale nei dati non è stata oggetto di verifica in questa analisi. Si segnala inoltre che, essendo il test applicato in modo indipendente su un numero elevato di unità territoriali (Comuni), il numero di trend risultati significativi per puro effetto del caso può aumentare a causa dei confronti multipli. E' quindi necessario precisare che si tratta di un aspetto non corretto nella presente analisi (es. mediante procedura di *Benjamini-Hochberg* o *Bonferroni*), che va tenuto in attenta considerazione nella corretta interpretazione dei risultati.
