...
*Valutazione controfattuale dell’impatto sanitario da concentrazioni atmosferiche di $\text{NO}_2$ ad alta risoluzione spaziale e propagazione dell’incertezza. Il caso studio della Regione Veneto.*
...
*Counterfactual health impact assessment of high-resolution ambient $\text{NO}_2$ concentrations with uncertainty propagation. The case study of the Veneto Region.*
...
- *epidemiologica*, valutata estraendo con reinserimento, ad ogni iterazione di bootstrap, il Rischio Relativo ($RR$) da una distribuzione log-normale incentrata sul valore OMS ($RR = 1{.}05$);
...
Per tentare di superare la rigidità di una stima di tio deterministico ed estendere l'analisi a una distribuzione empirica della variabilità di stima, è stata implementata la procedura di *non-parametric bootstrap* descritta nella sezione @sec-methods. 
...
Dal punto di vista epidemiologico, la stima ottenuta dalla valutazione di impatto sanitario rappresenta il numero di *casi/anno teorici*, cioè i decessi evitabili nell'eventualità in cui tutte le sezioni censuarie della Reione Veneto siano esposte ad un livello di concentrazione ambientale di $NO_2 \leq 10\ \mu g~m^{-3}$, che rappresenta la soglia OMS stabilita dalle attuali evidenze scientifiche per escludere significativi (misurabili) effetti sanitari (malattie per cause respiratorie codici ICD-10: J00-J99).
...
L'analisi ad alta risoluzione spaziale affronta limiti strutturali propri degli studi ecologici:
...
- *Fenomeni di spillover spaziale*: il calcolo dei casi attribuibili su singola unità presuppone l'indipendenza tra aree contigue, un'ipotesi molto semplificativa rispetto alla natura diffusiva degli inquinanti atmosferici.

Per tentare di superare tali sfide, i principali sviluppi futuri indirizzeranno tre possibili direttrici metodologiche:

- *Inferenza causale spaziale (Spatial Causal Inference)*: adozione di un framework di inferenza causale [@dominici_cas_evi:2017] (tramite propensity score, IPTW, G-computation) esteso al contesto spaziale [@akbari_sp_inf:2023]. Tale approccio permetterà di isolare il ruolo dei confondenti socio-demografici e territoriali in assenza di dati individuali, formalizzando al contempo l'impatto delle esposizioni nelle aree limitrofe.
...
- *Propagazione integrata incertezza*: estensione dello schema probabilistico mediante l'inclusione esplicita dell'incertezza associata ai tassi epidemiologici di base e alle stime demografiche sezionali.
...
In @fig-map-no2-ts sono riprodotte le mappe di distribuzione spaziale delle concentrazioni medie annuali di $NO2$ da cui si nota un sensibile e stabile decremento nel tempo dei valori ambientali, con i livelli medi più bassi registrati nel 2025.
