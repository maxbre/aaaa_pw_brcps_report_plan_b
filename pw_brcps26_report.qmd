...
Per quantificare l'impatto sanitario di esposizione controfattuale ai livelli ambientali di *biossido di azoto* ($NO_2$) dovuto a mortalità per cause respiratorie nella popolazione della classe 30+, *residente nelle sezioni censuarie ISTAT* in Regione Veneto, è stata applicata la funzione concentrazione-risposta implementata nel software *WHO AirQ+* [@who_airq:2020].
...
- $RR = e^{\beta \cdot \Delta C}$: il *Rischio Relativo* per  differenziale di concentrazione $\Delta C$;
...
- $N$: la popolazione target di riferimento (popolazione  $30+$);

- $B$: il tasso basale grezzo dell'esito sanitario (mortalità per cause respiratorie);
...
L'applicazione della formula di stima di impatto alla popolazione 30+ delle sezioni censuarie del Veneto ha permesso la stima dei casi attribuibili in relazione a due differenti scenari (livelli di concentrazione ambientale):

- *stato attuale* ($E_{\text{reale}}$), considera la distribuzione osservata dell'esposizione ambientale all'inquinante $NO_2$ nel 2025; 

- *controfattuale* ($E_{\text{cft}}$), considera una esposizione pari a $NO_2 = 10 ~ \mu g~m^{-3}$ per tutte le sezioni censuarie superiori a tale valore soglia.

Il $\Delta C$ associato a ciascuna sezione censuaria è rappresentato dalla differenza tra la concentrazione ambientale osservata (*stato attuale*) ed il valore soglia (*controfattuale*) fissato in $10 \ \mu g/m^3$, che rappresenta il livello ambientale di riferimento per la media annuale previsto dalle linee guida OMS sulla qualità dell'aria [@who_aq:2021], al fine di escludere significativi effetti sulla salute.

*Propagazione incertezza (simulazione Monte Carlo)*

Al fine di distinguere l’incertezza epidemiologica dalla variabilità temporale dell’esposizione, sono state condotte due analisi probabilistiche separate. 

Nella prima, riferita specificamente al 2025, le concentrazioni modellate, la popolazione e i tassi di mortalità sono stati mantenuti fissi, mentre il rischio relativo è stato estratto da una distribuzione lognormale parametrizzata sulla stima centrale e sul relativo intervallo di confidenza OMS/HRAPIE-2. 
La distribuzione risultante rappresenta l’incertezza epidemiologica associata alla stima del carico sanitario nel 2025.

Nella seconda analisi, finalizzata a descrivere la variabilità storica del carico sanitario, a ogni iterazione è stato estratto con reinserimento uno degli scenari annuali di esposizione compresi tra il 2019 e il 2025. L’anno estratto è stato applicato simultaneamente a tutte le sezioni censuarie, preservando la configurazione spaziale delle concentrazioni osservata nel corrispondente anno. Anche in questa analisi il rischio relativo è stato estratto dalla relativa distribuzione lognormale. I percentili della distribuzione ottenuta costituiscono un intervallo di simulazione riferito alla combinazione tra variabilità storica dell’esposizione e incertezza epidemiologica e non un intervallo di confidenza specifico per l’anno 2025.


*Software*

Le analisi statistiche sono state effettuate in ambiente di programmazione *R (v. 4.5+)* [@R_manual:2022], con l'impiego di pacchetti dedicati: `tydyverse` e `viridis` per il *data wrangling* e per la restituzione grafica, `sf` e `terra` per l'analisi e la manipolazione dei dati georeferenziati.
...
L'impiego della sezione censuria come unità di valutazione garantisce, almeno in riferimento ai valori mediani, una granularità spaziale sufficientemente fine dell'entità dei gruppi di analisi della popolazione residente [^nota-ha]. Tuttavia, l'asimmetria e la multimodalità della distribuzione della popolazione impongono di considerare con attenzione la presenza di sezioni censuarie con dimensioni e densità molto eterogenee, non facilmente generalizzabili in termini di estensione spaziale e di popolazione residente.

[^nota-ha]: L'estensione della sezione *mediana* copre una superficie equivalente ad un quadrato di lato circa 170 m. 

In @fig-map-pop30p viene riprodotta la distribuzione spaziale della popolazione in classe d'età 30+ residente nelle sezioni censuarie della Regione Veneto (anno 2021). Le sezioni censuarie abitate da residenti in classe d'età 30+ rappresentano la quasi totalità delle sezioni in Regione Veneto considerato che solo 111 sezioni, pari a circa 0.25% del totale, è abitata da popolazione in classe d'età inferiore a 30 anni (in particolare la classe d'età 20-29, ISTAT 2021).
...
La @fig-map-attesi rappresenta il numero di casi attesi/anno per cause respiratorie (codici ICD-10: J00-J99), cioè  il numero di decessi teoricamente osservabili ogni anno calcolati sulla base della popolazione esposta 30+ (sezioni censuarie ISTAT anno 2021) ed i tassi grezzi provinciali (tasso ISTAT anno 2024 per causa di mortalità respiratoria).

E' necessario precisare che la @fig-map-pop30p e la @fig-map-attesi sono rappresentazioni geografiche di tipo "demografico" che restituiscono la popolazione *target* (a rischio) rispetto alla quale è stata effettuata la presente valutazione di impatto sanitario.
...
Per una valutazione comparata dei valori di concentrazione ambientale di $NO_2$ rispetto alla serie storica registrata dal 2019 si rimanda al grafico *boxplot* in @fig-no2-boxplot-prov che riporta anche l'indicazione del valore target OMS $10\ \mu g~m^{-3}$, utilizzato come soglia di valutazione controfattuale nella presente stima  di impatto sanitario.

Per contestualizzare in modo più preciso l'esposizione della popolazione 30+ residente nelle sezioni censuarie del Veneto rispetto ai livelli ambientali di $NO_2$, in @fig-exp-pop_prov viene riprodotta la curva cumulata della percentuale di popolazione soggetta ai corrispondenti  livelli di concentrazione ambientale. Nel grafico sono riprodotte per semplicità solo le curve relative al 2019 ed al 2025, stratificate per Provincia, che evidenziano un significativo trend di riduzione del livello di esposizione e contemporaneamente definiscono il "margine" necessario per raggiungere la soglia controfattuale OMS fissata in $10\ \mu g~m^{-3}$.

Per un inquadramento sistematico della serie storica delle concentrazioni ambientali di $NO_2$ stimate per il periodo dal 2019 al 2025 tramite la catena modellistica implementata nella Regione Veneto ed una valutazione del trend annuale aaggregato spazialmente a livello comunale si rimanda ai dati e grafici riportati in Appendice (@fig-map-no2-ts e @fig-map-mk-sen).
...
Il differenziale di esposizione tra il valore al 2025 di concentrazione ambientale di $NO_2$ in ciascuna sezione censuaria della Regione Veneto rispetto al valore controfattuale  di $10\ \mu g~m^{-3}$, previsto dalle linee guida OMS per escludere significativi effetti sulla salute, è risultato in media di $5.7\ \mu g~m^{-3}$, con un valore mediano di $5.9\ \mu g~m^{-3}$ ed un range interquartile di $3.9\ \mu g~m^{-3}$.

Il differenziale appare tuttavia significativamente diversificato se consideriamo la stratificazione su base provinciale riportata in @tbl-diff-prov.
...
La stima di impatto sanitario per ciascuna sezione censuaria è basata su tale  differenziale di esposizione tramite l'applicazione della funzione concentrazione risposta definita nella sezione -@sec-methods [Materiali e metodi](#sec-methods) ed analizzata in dettaglio nell'Appendice.
...
La stima dei casi attribuibili per l'anno di riferimento 2025, estesa a tutte le sezioni censuarie della Regione Veneto, restituisce un valore complessivo puntuale di *109 casi/anno per cause respiratorie (codici ICD-10: J00-J99) per la popolazione di età pari o superiore a 30 anni*. 
Assumendo il valore centrale del Rischio Relativo di letteratura OMS ($RR = 1{.}05$), la stima frazione attribuibile (AF) per l'intera regione è pari al 2.86% dei casi attesi. Applicando gli estremi dell'intervallo di incertezza del $RR$ ($1{.}03 - 1{.}07$), il carico di mortalità varia deterministicamente da un valore minimo di 66.3 casi/anno ($\text{AF}_{\text{low}} = 1{.}74\%$) a un massimo di 150.0 casi/anno ($\text{AF}_{\text{upp}} = 3{.}95\%$).

Nella @fig-map-sez-af viene riprodotta la frazione attribuibile (AF) nelle sezioni censuarie del Veneto, cioè il rapporto tra casi attribuibili e casi attesi annui che rappresenta la frazione di casi evitabili (decessi/anno) per malattie respiratorie potenzialmente raggiungibile attraverso l’azzeramento del differenziale di esposizione al valore controfattuale OMS ($10\ \mu g~m^{-3}$).
...
E' necessario precisare che gli estremi $\text{AC}_{\text{low}}$ e $\text{AC}_{\text{upp}}$ riportati nella @tbl-ac-prov rappresentano dei valori deterministici, calcolati direttamente dall'intervallo di variabilità del $RR$ da letteratura OMS ($1.03 - 1.07$). 

La stima puntuale riferita al 2025, ottenuta applicando il valore centrale del rischio relativo (1.05) è stata confrontata con una simulazione Monte Carlo nella quale il RR è stato estratto da una distribuzione lognormale parametrizzata sul relativo intervallo di confidenza. Poiché esposizione, popolazione e mortalità basale sono mantenute fisse, la simulazione propaga esclusivamente l’incertezza epidemiologica del RR. La sostanziale corrispondenza tra la stima deterministica e la mediana simulata, nonché tra gli estremi deterministici e i percentili della simulazione conferma la coerenza numerica dell’implementazione.

La @fig-sim-2025 illustra la curva di densità di probabilità associata alla distribuzione empirica dei casi attribuibili generata con il processo di simulazione relativo all'anno 2025.

La linea tratteggiata rossa identifica la mediana simulata pari a circa 109 casi/anno. Le due linee verticali tratteggiate in verde delimitano l'intervallo di simulazione al 95%: l'estremo inferiore ($\text{IC } 95\%\text{ inf.}$) si attesta in circa 68 casi/anno ($2{.}5^\circ$ percentile), mentre l'estremo superiore ($\text{IC } 95\%\text{ sup.}$) raggiunge circa 150 casi/anno ($97{.}5^\circ$ percentile). L'area ombreggiata sottesa alla curva racchiude la massa di probabilità centrale corrispondente all'intervallo di simulazione al 95%.

```{r}
#| label: fig-sim-2025
#| fig-cap: "Simulazione distibuzione (AC) casi attribuibili/anno in Regione Veneto nel 2025."
#| fig-align: "center"
...
Per la stima 2025 è stata applicata una simulazione Monte Carlo finalizzata alla propagazione dell’incertezza epidemiologica del rischio relativo. Separatamente, la variabilità delle esposizioni nel periodo 2019–2025 è stata valutata mediante una simulazione Monte Carlo con ricampionamento degli scenari annuali di esposizione. 
La simulazione storica aggiunge la variabilità dell’esposizione tra il 2019 e il 2025 e risponde ad una domanda diversa: "Qual è l'entità del carico sanitario sulla stessa popolazione e con gli stessi tassi di mortalità se l’esposizione ambientale risulta quella di un anno scelto casualmente tra il 2019 e il 2025?".

La @fig-sim-hist ... qui commento

```{r}
#| label: fig-sim-hist
#| fig-cap: "Simulazione distibuzione (AC) casi attribuibili/anno in Regione Veneto nel periodo 2019-2025."
#| fig-align: "center"
#| out-height: "0.6\\textheight"
#| out-width: "!\\textheight"
#| echo: false
...
L’intervallo della simulazione 2025 deriva solamente dal RR. La maggiore ampiezza dell’intervallo storico deriva anche dalle differenze di esposizione tra gli anni.

Poiché popolazione e tassi vengono mantenuti fissi, le differenze tra gli anni sono attribuibili esclusivamente alle diverse concentrazioni modellate di $NO_"$. È quindi una forma di standardizzazione utile per mostrare l’effetto del cambiamento ambientale.

La simulazione storica non modifica la stima specifica del 2025, ma la colloca nel contesto della variabilità ambientale osservata tra il 2019 e il 2025. Il confronto mostra quanto il carico sanitario attribuibile dipenda dalla scelta dell’anno di esposizione e consente di valutare se il 2025 rappresenti uno scenario tipico oppure particolarmente favorevole rispetto al periodo considerato.

## Discussione e conclusioni

Dal punto di vista epidemiologico, la stima ottenuta con la valutazione di impatto sanitario rappresenta il numero di *casi/anno teorici*, cioè i decessi evitabili nell'eventualità in cui tutte le sezioni censuarie della Regione Veneto siano esposte ad un livello di concentrazione ambientale di $NO_2 \leq 10\ \mu g~m^{-3}$, corrispondente al livello guida OMS 2021 per la media annuale assunta nel presente studio come concentrazione controfattuale di riferimento. Non rappresenta necessariamente una soglia biologica al di sotto della quale il rischio sanitario sia nullo.

----

La distribuzione ottenuta includendo gli anni 2019–2025 presenta un valore centrale superiore alla stima specifica del 2025, poiché incorpora anni caratterizzati da concentrazioni mediamente più elevate. Le due quantità rispondono pertanto a domande parzialmente differenti.

----

L'analisi dettagliata per l'anno 2025 evidenzia alcuni aspetti interessanti che derivano da una marcata eterogeneità territoriale del carico sanitario attribuibile (@tbl-ac-prov). 
Le province di Padova e Venezia concentrano la quota maggiore di casi attribuibili (rispettivamente circa 27 e 24 casi/anno, con AF pari al 3,5% e 3,8% dei casi attesi), coerentemente con i più alti differenziali di esposizione rispetto alla soglia controfattuale OMS (@tbl-diff-prov). Al contrario, la Provincia di Belluno presenta un impatto sanitario pressoché trascurabile (AF = 0,1%), risultato atteso vista la ridotta pressione emissiva e le caratteristiche orografiche che la rendono un territorio meno soggetto all'accumulo inquinante tipico delle zone di pianura. 

Questa distribuzione disomogenea dei casi attesi conferma che eventuali politiche regionali di riduzione dell'esposizione a $NO_2$ potrebbero ottenere il maggior beneficio sanitario se prioritariamente indirizzate alle aree di pianura a maggiore pressione antropica (densità abitativa, riscaldamento domestico, traffico veicolare, insediamenti produttivi), rispetto all'applicazione di un approccio indifferenziato sull'intero territorio regionale.
...
Le principali caratteristiche e limitazioni dello studio di impatto sanitario riguardano i seguenti aspetti:

- *natura ecologica*: l'unità di analisi è rappresentata dalla sezione censuaria e quindi i risultati vanno interpretati a livello aggregato di popolazione, evitando l'inferenza diretta a livello individuale (*ecological fallacy*); in particolare, si rileva che tutti i confondenti di tipo individuale rimangono non risolti fino a quando si opera nell'ambito di stime di gruppo;

- *trasportabilità parametri di stima*: sia il Rischio Relativo (derivato dalla letteratura internazionale OMS/HRAPIE-2), sia i tassi di mortalità di base (stimati a livello provinciale e attribuiti alle sezioni censuarie) presuppongono una trasportabilità che non tiene conto di eventuali differenze locali nella suscettibilità della popolazione, nella composizione demografica o nella qualità delle cure sanitarie;

- *aggregazione spaziale e sovrastima sistematica*: come dimostrato in Appendice, calcolare l'impatto sanitario su dati di esposizione pre-aggregati porterebbe a una sovrastima sistematica rispetto a un'analisi condotta alla massima risoluzione disponibile; per questo motivo la stima dei casi attribuibili è stata effettuata a livello di singola sezione censuaria prima di ogni aggregazione, mitigando questo tipo di *bias*;
...
- *singolo inquinante*: la valutazione si concentra esclusivamente su $NO_2$, mentre nella realtà la popolazione è co-esposta ad una miscela di inquinanti (in particolare $PM_{2.5}$) con possibili effetti sinergici e/o di confondimento non catturabili dal presente modello deterministico concentrazione-risposta;
...
- *autocorrelazione spaziale*: la stima bootstrap assume una completa indipendenza tra le sezioni censuarie e necessariamente non riesce a catturare l'eventuale dipendenza spaziale tra sezioni vicine;

- *autocorrelazione temporale*: i valori di esposizione osservati in anni successivi per una medesima sezione censuaria non sono tra loro indipendenti, ma presentano autocorrelazione temporale che l'attuale procedura di bootstrap tenta di catturare tramite il ricampionamento della serie storica 2019-2025, senza tuttavia modellarla esplicitamente attraverso un impianto di serie storiche o processi spazio-temporali dedicati.

*Conclusioni metodologiche e possibili sviluppi futuri*

Il percorso metodologico proposto — basato sulla disaggregazione a livello di sezione censuaria e sulla propagazione dell'incertezza tramite bootstrap — costituisce una struttura replicabile a supporto della programmazione regionale in materia di Ambiente e Sanità Pubblica.

L'analisi affronta tuttavia limiti strutturali propri degli studi ecologici:

- *confondimento spaziale e collinearità:* difficoltà nell'isolare l'impatto sanitario dalle caratteristiche socio-demografiche e territoriali [@ramsey_ps:2019].

- *Modifiable Areal Unit Problem (MAUP):* distorsione legata all'*effetto di scala* (l'aggregazione dei dati appiattisce le variabilità locali) e all'*effetto di zonalità* (i confini amministrativi non riflettono la reale dispersione fisica del $\text{NO}_2$).

- *spillover spaziale:* il calcolo dei casi attribuibili su singola unità presuppone l'indipendenza tra aree contigue, un'ipotesi semplificativa rispetto alla natura diffusiva degli inquinanti atmosferici.

Per superare tali sfide, i principali sviluppi futuri indirizzeranno tre possibili direttrici metodologiche:

- *inferenza causale spaziale (Spatial Causal Inference):* adozione di un framework di inferenza causale [@dominici_cas_evi:2017], tramite *propensity score*, IPTW o *g-computation* esteso al contesto spaziale [@akbari_sp_inf:2023], per isolare il ruolo dei confondenti socio-demografici in assenza di dati individuali;

- *modellazione bayesiana e autocorrelazione:* impiego di *Empirical Bayes Smoothing* per stabilizzare i tassi di mortalità nelle piccole aree, integrato da strutture autoregressive spaziali (modelli CAR/MICAR, Besag-York-Mollié) e indici di autocorrelazione ($I$ di Moran) [@blangiardo_cameletti:2015] per modellare la dipendenza di vicinato e la covarianza residua.

- *propagazione integrata incertezza:* estensione dello schema probabilistico di bootstrap mediante l'inclusione esplicita dell'incertezza associata ai tassi epidemiologici di base e alle stime demografiche sezionali.
...
Questa sezione dell'Appendice riprende e sviluppa la nota *[1]* riportata nel paragrafo  -@sec-methods [Materiali e metodi](#sec-methods), fornendo un inquadramento rispetto alle caratteristiche della funzione di stima dei casi attribuibili $AC$, e conseguentemente della frazione attribuibile $AF$.
...
La derivata prima $f'(\Delta C) > 0$ è strettamente positiva, e quindi, la funzione è *monotonicamente crescente*: all'aumentare di $\Delta C$ inquinante (riduzione rispetto allo stato attuale), i casi evitati aumentano. 
La derivata seconda $f''(\Delta C) < 0$ è strettamente negativa, e quindi, la funzione è *strettamente concava*, cioè con la curvatura rivolta verso il basso, cioè caratterizzata da rendimenti marginali decrescenti.

In altri termini, piccole riduzioni iniziali delle concentrazioni di inquinante generano benefici sanitari più elevati rispetto a riduzioni successive della stessa entità, creando una crescita iniziale molto ripida dell'impatto evitato che poi tende a stabilizzarsi (appiattirsi) quando il $\Delta C$ aumenta ulteriormente.
...
Poiché $f(\Delta C)$ è strettamente concava, come dimostrato in precedenza, possiamo applicare questa disuguaglianza ponendo $\varphi = f$ e trattando $\Delta C$ come una variabile casuale che descrive la *variabilità spaziale* dell'esposizione tra le unità di analisi che compongono l'area di studio:
...
- **$f(E[\Delta C])$**: il beneficio calcolato applicando la formula un'unica volta alla riduzione *media complessiva* dell'area, trattando l'intera popolazione come se fosse esposta in modo omogeneo a quel valore medio.
...
($\overline{\Delta C} = 10$) e $\beta = 0{,}01$.  Si ottiene:
...
cioè, la stima aggregata risulta, seppur di poco, superiore.
...
In @fig-map-no2-ts sono riprodotte le mappe di distribuzione spaziale delle concentrazioni medie annuali di $NO_2$ da cui si nota un sensibile e stabile decremento nel tempo dei valori ambientali, con i livelli medi più bassi registrati nel 2025.
...
La @fig-map-mk-sen riproduce la distribuzione geografica dei risultati del test non parametrico di *Mann-Kendall* e della stima della pendenza di Sen (*Sens'slope*) per le concentrazioni ambientali di $NO_2$ (medie annuali) raggruppate per Comune in Regione Veneto. Nella mappa i colori all'interno dei Comuni indicano la pendenza della retta non parametrica, con valori negativi (in colore graduato blu) del coefficiente angolare che indica un *trend decrescente nel tempo*, mentre i confini amministrativi dei Comuni in evidenza (con bordo ispessito) individuano un trend temporale statisticamente significativo ($\alpha = 0.05$).

```{r}
#| label: fig-map-mk-sen
#| fig-cap: "Trend annuale $NO_2$ comunale (2019-2025). Per una dettagliata descrizione dei colori e della simbologia utilizzata nella rappresentazione si rimanda al testo."
#| fig-align: "center"
#| out-height: "0.5\\textheight"
#| out-width: "!\\textheight"
#| echo: false
...
Al fine di contenere l'inflazione dei falsi positivi derivante dall'esecuzione simultanea dei test su tutti i Comuni della Regione Veneto, i $p$-value di Mann-Kendall sono stati corretti mediante la procedura di Benjamini-Hochberg (False Discovery Rate - FDR), garantendo un bilanciamento ottimale tra il controllo delle false scoperte e la potenza statistica della stima della pendenza di Sen. 
Si segnala tuttavia che l'analisi non include una verifica della struttura di autocorrelazione spaziale e temporale nei dati e pertanto, l'attuale rappresentazione cartografica in @fig-map-mk-sen sottende un preciso limite metodologico e va interpretata con molta cautela, attribuendole un valore esclusivamente indicativo.
