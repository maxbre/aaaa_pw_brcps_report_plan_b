...
Nella @fig-map-sez-delta-no2 viene rappresentata per le sezioni censuarie del Veneto la distribuzione spaziale del differenziale di concentrazione di $NO_2$ tra il valore attuale del 2025 ed il valore controfattuale previsto dalle linee guida OMS fissato nella soglia di $10\ \mu g~m^{-3}$. 

La stima di impatto sanitario per ciascuna sezione censuaria è basata su questo differenziale tramite l'applicazione della funzione concentrazione risposta definita nella sezione -@sec-methods [Materiali e metodi](#sec-methods) ed analizzata in dettaglio nell'Appendice.
...
In termini di sensitività dei risultati, è particolarmente significativo apprezzare l'entità della variabilità introdotta dal processo di bootstrapping spazio-temporale *completo con clustering comunale* rispetto a un approccio *"semplice" (naive)* che considera le sezioni censuarie come totalmente indipendenti e solo per le concentraizoni ambientali riferita all'anno 2025.

In @tbl-boot-comp sono riportate le principali metriche di confronto dei risultati del bootstrap eseguito con due modalità alternative: semplice (osservazioni indipendenti riferite al solo anno 2025) e spazio-temporale (clustering comunale e perturbazione temporale anni 2019-2025.
...
: Confronto tra le prestazioni analitiche del bootstrap semplice e spazio-temporale. {#tbl-boot-comp}
...
L'analisi evidenzia  una marcata eterogeneità territoriale del carico sanitario attribuibile (@tbl-ac-prov, @tbl-diff-prov). 
...
- l'impiego di uno smussamento bayesiano empirico (*Empirical Bayes Smoothing*) per la stima del tasso di mortalità rifierito a piccole unità territoria (comuni/sezioni), eventualmente integrato con la modellazione della dipendenza spaziale;
...
Un ulteriore limite strutturale del disegno ecologico riguarda il *Modifiable Areal Unit Problem* (MAUP). La modifica della scala geografica o dei confini delle unità territoriali di analisi può alterare sensibilmente i risultati statistici. Questo si manifesta su due livelli distinti:
...
Pur non trattandosi di un disegno causale in senso stretto, tale limite concettuale - riconducibile alla più generale ipotesi di indipendenza tra unità propria tipica dei dei framework controfattuali - offre una chiave di lettura nuovo per inquadrare l'approccio ecologico adottato e per orientare i possibili sviluppi metodologici. 
